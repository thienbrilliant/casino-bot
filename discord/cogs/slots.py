...
class Slots(commands.Cog):
    ...
    @commands.command(
        brief='Máy đánh bạc (slot)\nTiền cược phải từ 1–3',
        usage='slots *[tiền cược]'
    )
    async def slots(self, ctx: commands.Context, bet: int = 1):
        ...
        embed = make_embed(
            title=(
                f'Bạn {result[0]} {result[1]} credits' +
                ('.' if result[0] == 'thua' else '!')
            ),
            description=(
                'Hiện bạn có ' +
            ...
            color=(
                discord.Color.red() if result[0] == "thua"
    ...
    @commands.command(
        brief=f"Mua credits. Mỗi credit trị giá ${DEFAULT_BET}.",
        usage="buyc [credits]",
        aliases=["buy", "b"]
    )
    ...
    @commands.command(
        brief=f'Bán credits. Mỗi credit trị giá ${DEFAULT_BET}.',
        usage="sellc [credits]",
        aliases=["sell", "s"]
    )
